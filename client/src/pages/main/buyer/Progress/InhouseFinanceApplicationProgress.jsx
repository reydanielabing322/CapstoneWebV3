...
function InhouseFinanceApplicationProgress({ application }) {
    return (
                    ...
                    <tr className={`p-2 border border-b-black flex flex-row gap-2 items-center ${application.progress >= 3 && 'bg-[#34F481]'} ${application.status == 'rejected' && 'bg-red-500'}`}>
                        <td>
                            {application.status == "on-going" ? (
                                <>
                                    {application.progress < 2 ? (
                                        <p>3. Dealership Credit Investigation on process.</p>
                                    ) : (
                                        <p>3. Dealership Credit Investigation Verified.</p>
                                    )}
                                </>
                            ) : (
                                <>
                                    {application.progress == 2 ? (
                                        <p className='text-white'>3. Credit Investigation Rejected.</p>
                                    ) : application.progress >= 2 ? (
                                        <p>3. Dealership Credit Investigation Verified.</p>
                                    ) : (
                                        <p>3. Dealership Credit Investigation on process.</p>
