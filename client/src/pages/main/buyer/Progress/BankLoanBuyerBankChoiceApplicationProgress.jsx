...
function BankLoanBuyerBankChoiceApplicationProgress({ application }) {
    return (
                    ...
                    <tr className={`p-2 border border-b-black flex flex-row gap-2 items-center ${application.progress >= 3 && 'bg-[#34F481]'} ${application.status == 'rejected' && 'bg-red-500'}`}>
                        <td>
                            {application.status == "on-going" ? (
                                <>
                                    {application.progress < 2 ? (
                                        <p>3. Bank Loan Certificate on verification.</p>
                                    ) : (
                                        <p>3. Bank Loan Certificate verified.</p>
                                    )}
                                </>
                            ) : (
                                <>
                                    {application.progress == 2 ? (
                                        <p className='text-white'>3. Bank Loan Certificate rejected.</p>
                                    ) : application.progress >= 2 ? (
                                        <p>3. Bank Loan Certificate verified.</p>
                                    ) : (
                                        <p>3. Bank Loan Certificate on verification.</p>
